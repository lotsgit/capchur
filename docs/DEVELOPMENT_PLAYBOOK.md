...
| 2026-08-08 | Prepare one short-lived visible-tab screenshot when a native select opens and attach it only if the selection commits.                                                                                     | Post-change capture occurs after the browser closes its native menu, while general pointer-dwell capture would exceed browser screenshot quotas and collect unnecessary images. | The worker keeps at most one in-memory candidate per tab for 10 seconds, persists no candidate unless a matching select change occurs, and falls back to normal post-action capture when no valid candidate exists.                                                                            |
