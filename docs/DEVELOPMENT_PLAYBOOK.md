...
- [x] Reduce first use to one named Connect & sync confirmation and automatically enqueue stopped sessions and review changes.

**Acceptance:** Record offline, reconnect, upload once, and open the correct guide in the editor.

**Validation evidence:** The original S11 acceptance passed with 79 tests. The first-use refinement adds focused contract, durable queue, and database-backed authorization coverage for queue-before-authorization, automatic flush, canonical account-name confirmation, idempotent automatic sync, offline retry, and token expiry. Repository typecheck and lint pass; cross-browser package and interaction checks remain required before release.
...
**Validation evidence:** The original durable export acceptance passed with 95 tests. The professional presentation enhancement passed all 123 workspace tests, including a real Chromium-rendered A4 50-step PDF parsed for page size and pagination and a 50-step DOCX with all 100 expected image resolutions. OpenXML tests verify named styles, A4 dimensions, a title page, running headers and page fields, bookmarks, internal links, non-splitting rows, captions, and field refresh settings. PDF tests cover a separate print-only cover, contents, section, and framed-step template while portable HTML and Markdown remain unchanged. Repository typecheck, lint, production builds, and the high-severity dependency audit passed; Microsoft Word, LibreOffice, and browser print inspection remain release-checklist items.
...
- [x] Replace the fixture-only signed-in entry with a persisted guide dashboard and active capture, help, settings, export, and password-recovery journeys.
...
| S18 Release            | NEXT   | Release checklist | Automated gates and backup restore pass; rollback and operator sign-off remain. |
...
| 2026-08-03 | Authorize the extension through browser identity using one-time codes and hashed one-hour bearer tokens, and sync through a durable worker-owned queue.                                                     | Page context must never receive extension credentials, while MV3 suspension, offline recording, and partial image uploads require persisted resumable state.         | First use queues the stopped session before one Connect & sync gesture, completes the signed-in web callback automatically, confirms the account name, and flushes; later stops and review changes enqueue automatically, while retry, conflict, expiry, and local durability remain explicit. |
...
| 2026-08-06 | Render PDF and DOCX with one deterministic professional A4 theme while keeping portable HTML/Markdown and export job contracts unchanged.                                                                   | A consistent default improves deliverable quality without adding premature presets, settings persistence, or a database migration.                                   | PDF uses a dedicated print-only cover, contents, section, framed-step, and running-page template; DOCX uses named styles, a title page, static clickable contents, framed steps, captions, headers, and page fields; pagination remains adaptive.                                              |
...
| 2026-08-08 | Prepare one short-lived visible-tab screenshot when a native select opens or the pointer dwells over an ARIA option/menu item, retaining an inert menu visual through a matching option click.                              | Chrome may queue screenshot processing until after application-rendered menus close, while general pointer-dwell capture would exceed browser quotas and collect unnecessary images. | The worker keeps at most one target-matched candidate per tab for 10 seconds; ARIA menus are visually retained only until the matching capture response, no candidate is persisted unless the action commits, and normal post-action capture remains the fallback.                                  |
