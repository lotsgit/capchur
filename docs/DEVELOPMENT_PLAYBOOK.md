...
**Current milestone:** `S09 - Database, object storage, and API`

**Current repository status:** Runtime-validated capture and guide contracts, persistent recording state, popup controls, pure element analysis, approved-page click capture, visible-tab screenshots, local session review, and a responsive fixture-backed web guide editor are implemented with focused Vitest coverage. Users can edit and reorder guide steps with selected-step media annotations on desktop and mobile before cloud persistence is introduced.

**Last completed session:** `S08 - Web guide editor foundation`

**Next action:** Complete S09 only. Add trusted persistence boundaries for guides and images using the shared guide model established in S08.
...
| S09 Persistence API    | NEXT    | -                 | S08 guide model is ready for trusted persistence boundaries.     |
