# Release Checklist

Release candidate: 0.1.0. Production origin: `https://capchur.io`.

## Automated Gates

Run from a clean checkout with Node 22+ and pnpm 11:

```powershell
corepack pnpm install --frozen-lockfile
corepack pnpm run install:pdf-browser
corepack pnpm run install:extension-browsers
corepack pnpm run test
corepack pnpm run typecheck
corepack pnpm run lint
corepack pnpm run build
$env:WXT_WEB_ORIGIN = "https://capchur.io"
corepack pnpm run build:extension:browsers
corepack pnpm --filter @capchur/extension test:browser
corepack pnpm audit --audit-level high
corepack pnpm run verify:release
corepack pnpm run zip:extension:browsers
```

Start the production web build with release-equivalent non-production secrets and isolated data, then run:

```powershell
$env:CAPCHUR_WEB_URL = "http://127.0.0.1:3100"
corepack pnpm run test:release:web
```

The browser gate requires zero serious/critical Axe findings, no browser errors or horizontal overflow at 1440x900 and 390x844, a load under 4 seconds, and transferred resources under 2 MiB. The package gate requires the exact approved manifests, HTTPS production origin, no unreviewed dynamic execution, and each unpacked extension under 5 MiB.

## Manual Rehearsal

- [ ] Install the unpacked Chrome/Edge package and signed Firefox candidate in clean profiles.
- [ ] Verify the public home, Privacy, extension installation links, Help, and mobile navigation.
- [ ] Create an account, show/hide the password, recover it by email, sign in, and change it from Settings.
- [ ] Confirm a new account opens an empty guide dashboard rather than fixture content.
- [ ] Connect the extension and verify denied permission and protected-page messaging.
- [ ] Record at least five actions across navigation; confirm values/passwords are absent and screenshots survive restart.
- [ ] Review locally, sync once after simulated offline use, edit/reorder/redact, and resolve one stale edit conflict.
- [ ] Verify private/workspace/revocable-link access and immediate revocation in a second profile.
- [ ] Export HTML and Markdown and confirm their portable layout and local assets remain unchanged.
- [ ] Inspect 1-step, 5-step, and 50-step professional A4 PDFs in Firefox and Chrome: cover, contents links, section breaks, framed screenshots, headers, page numbers, adaptive pagination, links, highlights, and redactions.
- [ ] Inspect the same DOCX guides in Microsoft Word and LibreOffice: title page, clickable contents, named heading hierarchy, headers, `Page X of Y`, non-splitting step blocks, captions, image aspect ratios, links, highlights, and redactions.
- [ ] Confirm synced and manually created guides appear in the dashboard and PDF/DOCX controls are enabled after save.
- [ ] Delete the rehearsal guide/session and verify source and export objects are unavailable.
- [ ] Verify AI opt-in, redaction, timeout fallback, provider-off behavior, and usage-only logging.
- [ ] Confirm production TLS, private bucket policy, secret injection, alerts, backup freshness, and support/security/privacy mailboxes.
- [x] Complete an isolated backup restore and record recovery point and duration.
- [x] Run `infra/aws/rehearse-rollback.ps1` with the previous application image; record successful rollback and current-image restoration health checks.
- [x] Inspect both installable archives for secrets, source maps, development origins, and generated-only files.
- [ ] Record store validation results, package hashes, approver, date, and known-risk disposition below.

## Evidence

| Evidence                            | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unit/integration/type/lint/build    | Passed 2026-08-10: 136 tests; typecheck, lint, and production builds passed. A dependency/output-free temporary copy also completed a fresh 821-package `corepack pnpm install --frozen-lockfile` on 2026-08-11; its follow-on typecheck was interrupted by a terminal collision and is not counted as separate evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| High/critical dependency audit      | Rechecked 2026-08-10: patched `js-yaml` to 4.3.1 and Nano ID 3.x to 3.3.17. Two high-severity `image-size` 2.0.2 denial-of-service advisories remain formally accepted because upstream is archived with no patched release and the transitive package is used only by Mozilla `addons-linter` on repository-controlled extension icons, outside Capchur runtime and upload paths.                                                                                                                                                                                                                                                                                                                                                                                                            |
| Chromium/Edge/Firefox capture E2E   | Passed again 2026-08-11 against release-origin production bundles                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Axe and performance budgets         | Passed 2026-08-10 against production: desktop 3000 ms / 267615 bytes; mobile 1424 ms / 267617 bytes; zero serious/critical Axe findings, browser errors, or horizontal overflow                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Package manifest/safety/size        | Passed again 2026-08-11: Chrome 502371 bytes; Firefox 502593 bytes. Both 15-entry installable archives contain no environment files, dependencies, Git metadata, source maps, development origins, private-key markers, or source-map directives. The separate Firefox reproduction archive intentionally contains source-level development defaults and is not an installable package.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| AWS production deployment           | Passed 2026-08-04 in `ap-south-1`: CloudFormation `CREATE_COMPLETE`; ECR image `2041aa65d33c-20260804185217`; ECS steady with one healthy ALB target; ACM issued for `capchur.io`; HTTPS health returned 200; RDS private/encrypted with 30-day backups and deletion protection; S3 private/versioned; daily AWS Backup plan, three `OK` alarms, and the confirmed `prashant@bizleader.in` SNS subscription verified.                                                                                                                                                                                                                                                                                                                                                                                                     |
| Backup restore rehearsal            | Passed 2026-08-05 in `ap-south-1`: restored post-canary RDS recovery point `arn:aws:rds:ap-south-1:416107214402:snapshot:awsbackup:job-de704120-957c-4b70-abf5-216a8ce6519d` and S3 recovery point `arn:aws:backup:ap-south-1:416107214402:recovery-point:capchur-production-objectbucket-3lxcjnnc-20260805055316-0562c038` into an isolated no-ingress environment. The validator confirmed the synthetic guide, one revision, object metadata, exact SHA-256, and cross-workspace isolation. Overall restore time was 9m44s, within the 4-hour RTO. The restored RDS instance, versioned bucket, validator, IAM roles, VPC, log group, synthetic guide/object, and temporary credential secret were removed. Two empty synthetic account/workspace records remain because the product has no account-deletion workflow. |
| Application rollback rehearsal      | Passed 2026-08-10 in `ap-south-1`: deployed immutable previous image `25246c253104-20260806191854`, reached stable ECS and HTTPS `status: ok`, restored original image `1cd83690ebb6-20260807110741` in `finally`, and independently verified the restored service healthy on task definition `capchur-production-web:7`                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Production export rehearsal         | Passed 2026-08-10 for a persisted 5-step guide through the production UI and durable job queue. DOCX opened read-only in Microsoft Word without repair, rendered 3 pages, and contained the expected Contents, Prepare, Complete, and five step headings. PDF parsed as 5 A4 pages with the expected title. The separate 1-step, 50-step, LibreOffice, and cross-browser visual checks remain pending.                                                                                                                                                                                                                                                                                                                                                              |
| Store archive hashes and validation | Packages generated and locally verified 2026-08-10; Firefox minimum 142 and required consent metadata passed `web-ext lint` with zero errors and five reviewed generated-bundle warnings. Chrome `437c2588fea3d0cf516bb6270586625600d5ba84bbd9541079b94cea839c415f`, Firefox `2278849956fead256eba3be5ddab642229fa90fe4c2ab55e73d818d628fb15ab`, sources `7ba7a1ba8778d194efe57bd2acc0f3e64fd65a71cb366b6a36096a72f1913c25`; signed-candidate/store validation pending                                                                                                                                                                                                                                                                                         |

## Remaining Release Blockers

- Production reset-email delivery is not configured: `ResetEmailSecretArn` and `EmailFrom` are empty and no `capchur-production-reset-email` secret exists. The privacy-preserving UI response works, but production logs confirm no message is delivered.
- Chrome and Firefox store URLs, signed candidates, store review, and operator approval are not yet available.
- Firefox signed-package clean-profile validation and the full manual permission, offline sync, conflict, collaboration, HTML/Markdown, AI-provider, and 1/50-step export matrix remain pending.
- LibreOffice is unavailable on the release workstation; Microsoft Word 16 validation passed for the 5-step DOCX.

Release is blocked by any unexplained high-risk finding, failed restore, missing rollback evidence, unexpected permission/origin, or incomplete privacy/store metadata.
