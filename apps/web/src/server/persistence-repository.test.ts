...
describe("persistence repository", () => {
  ...
  it("supports guide CRUD while hiding another owner's records", async () => {
    const repository = createPersistenceRepository(handle);
    const created = await repository.createGuide(ownerId, guideId, write("Draft"), 100);

    expect(created.title).toBe("Draft");
    expect(await repository.getGuide(otherOwnerId, guideId)).toBeNull();
    expect(await repository.updateGuide(otherOwnerId, guideId, write("Stolen"), 200)).toBeNull();

    const updated = await repository.updateGuide(ownerId, guideId, write("Published"), 200);
    ...
    expect((await restartedRepository.getGuide(ownerId, guideId))?.title).toBe("Published");

    expect(await restartedRepository.deleteGuide(otherOwnerId, guideId)).toEqual([]);
    await restartedRepository.deleteGuide(ownerId, guideId);
    expect(await restartedRepository.getGuide(ownerId, guideId)).toBeNull();
  }, 60_000);

  it("rolls back guide metadata when step replacement fails", async () => {
    const repository = createPersistenceRepository(handle);
    await repository.createGuide(ownerId, guideId, write("Original"), 100);
    await repository.createGuide(ownerId, otherGuideId, write("Other", otherStepId), 100);

    await expect(
      repository.updateGuide(ownerId, guideId, write("Broken", otherStepId), 200),
    ).rejects.toThrow();

    const persisted = await repository.getGuide(ownerId, guideId);
